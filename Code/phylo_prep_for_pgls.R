...
moltree <- read.tree(here::here("Data/GBOTB.tre"))
moltree <- multi2di(moltree)

full.data <- read.csv(here::here("Data/20250211_LongFormat_croparchitecture_V3.csv"))

pruned.tree <- drop.tip(moltree, moltree$tip.label[!(moltree$tip.label %in% unique(full.data$Species.ssp.var))])

unique.crops <- unique(full.data$common.name[-grep("wild relative", full.data$common.name)])

triplets <- vector(mode = "list", length = length(unique.crops))
for(i in 1:length(triplets)){
    triplets[[i]] <- unique(full.data$Species.ssp.var[grep(unique.crops[i], full.data$common.name)])
}
